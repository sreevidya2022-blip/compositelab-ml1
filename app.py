...
- Buckling Load (N/m): measures resistance of the composite to structural instability under compression

ML Models used:
- GPR (Gaussian Process Regression): probabilistic model, trained on experimental points, gives real uncertainty estimates
- GBM (Gradient Boosting Machine): ensemble of 60 decision trees trained on weighted data
- Auto-selection: model with higher R² is chosen per property

Dataset: experimental points + theory data + FEA simulation data.
Experimental data carries 90% weight, theory+FEA carry 10%.

Valid input range: BN and AO both between 2.5% and 7.5%.

Buckling range: 500 – 1200 N/m.
